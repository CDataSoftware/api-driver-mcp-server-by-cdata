...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for API Driver (beta)](https://www.cdata.com/download/download.aspx?sku=FAZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data API Driver supported by the [CData JDBC Driver for API Driver](https://www.cdata.com/drivers/apidriver/jdbc).
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/apidriver/download/jdbc](https://www.cdata.com/drivers/apidriver/download/jdbc)
